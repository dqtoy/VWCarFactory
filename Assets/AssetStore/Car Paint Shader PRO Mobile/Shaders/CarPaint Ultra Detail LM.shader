...
SubShader {
   Tags { "QUEUE"="Geometry" "RenderType"="Opaque" " IgnoreProjector"="True"}	  
      Pass {  
         ...
         fixed4 frag(vertexOutput input) : COLOR
         {
		    ...
		    //fixed4 encodedNormal = tex2D(_BumpMap, _BumpMap_ST.xy * i.tex.xy + _BumpMap_ST.zw);
			...
			fixed dotLN = dot(lightDirection, input.normalDir); 				  
               ...
               float3 binormalDirection = 
				cross(input.normalDir, input.tangentWorld);					  
			...
			//specularReflection *= saturate(Luminance(lm.rgb));
 ...
 //flakes
   ...
   paintColor *= saturate(Luminance(lm.rgb));
			...
			reflTex.rgb *= saturate(Luminance(lm.rgb));
			...
			fixed4 color = fixed4(textureColor.rgb * saturate(input.vertexLighting + ambientLighting + diffuseReflection) * saturate(Luminance(lm.rgb)) + specularReflection, 1.0);
